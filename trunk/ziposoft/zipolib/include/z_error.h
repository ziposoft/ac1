...
#define	Z_ERROR(status)   (z_debug_break()?get_logger().add_msg (z_logger_lvl_error,__FILE__,__FUNCTION__,__LINE__,status,0):0);
#define	Z_ERROR_MSG(status,...)    (z_debug_break()?get_logger().add_msg (z_logger_lvl_error,__FILE__,__FUNCTION__,__LINE__,status,__VA_ARGS__):0);
